...
const EditorPane = () => {
    ...
    const width = {
        pc: 'w-full',
        tablet: 'w-[768px] editor-tablet',
        phone: 'w-[375px] editor-phone'
    }
    const {blocks, current, setCurrent, refresh} = useBuilder()

    useEffect(() => {
        window.frames['editor_iframe'].contentWindow.document.head.innerHTML = document.head.innerHTML
    }, [refresh])
    ...
    return (
        <>
            ...
            <div
                className={`bg-white ${width[monitor] || ""} mx-auto transition-all overflow-auto`}
            >
                <Iframe id="editor_iframe" className="w-full h-[calc(100vh-115px)]">
                    <div className="w-full min-h-screen" onDrop={(e) => {
                        let index = -1
                        let editor = window.frames['editor_iframe'].contentWindow.document.querySelector("#block-editor-pane")
                        ...
                        <div className="block-editor slim-scroll" id="block-editor-pane">
